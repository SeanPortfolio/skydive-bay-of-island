...
function save_item()
{
    ...
    $video_id   = $_POST['photo-video-id'];
    $caption    = $_POST['photo-caption'];
    $alt        = $_POST['photo-alt'];
    ...
    if( !empty($full_path) )
    {
        ...
        for($i=0; $i < count($full_path); $i++)
        {
            ...
            if( is_file($photo_full_path) )
            {
                ...
                $resizer_class->resizer($upload_dir_full_path, $photo_full_path, 430, 395, $thumb_name);
                ...
                $photo_data['video_id']       = $video_id[$i];
                $photo_data['caption']        = $caption[$i];
                $photo_data['alt_text']       = $alt[$i];
