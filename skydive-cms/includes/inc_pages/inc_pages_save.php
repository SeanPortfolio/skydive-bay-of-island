...
function save_item (){
    ...
    $is_top_position    		= sanitize_input('is_top_position'); 
	...
	$meta_data['jump_gallery_id']      = sanitize_input('jump_gallery_id', FILTER_SANITIZE_NUMBER_INT);
	// $meta_data['is_top_position']      = ($is_top_position === FLAG_YES) ? FLAG_YES : FLAG_NO;
	$meta_data['icon']                 = sanitize_input('icon'); 
	$meta_data['page_youtube_id']      = sanitize_input('page_youtube_id');  
	...
	// save testimonial
	run_query("DELETE FROM `page_has_review` WHERE `page_id` = '{$id}'");

	$testimonials = $_POST['testimonial_id'];
	...
	if( !empty($testimonials) )
	{
		for($i=0;$i<count($testimonials);$i++)
		{
			...
			$ql_arr['review_id'] 		= $testimonials[$i];
			$ql_arr['page_id']      	= $meta_data_id;

			insert_row($ql_arr, 'page_has_review');
	...
	##------------------------------------------------------------------------------------------------------
    ##   Save specials
    run_query("DELETE FROM `page_has_special` WHERE `page_id` = '{$id}'");

    $special_ids = $_POST['special_id'];
    ...
    if( $special_ids )
    {
        ...
        { 
            ...
            $special_id = $special_ids[$i];

            $ins_query .= ',('.$id.','.$special_id.')';
        }

        $ins_query = ltrim($ins_query, ',');

        if( $ins_query )
        {
            $final_query = "INSERT INTO `page_has_special`(`page_id`, `special_id`) VALUES {$ins_query};";
