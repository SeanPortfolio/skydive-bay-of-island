	...
	$posts_view = <<<HTML
		<article class="blog__item">
		 	<div class="blog__item__img"  title="{$post['title']}">
		 		<a href="{$blog_full_url}" title="{$post['title']}" class="zoom__wrapper rounded__border">
		 		 	<span style="background-image:url({$post['thumb_photo_path']})" class="zoom"></span>
		 		</a>
		 	</div>
		 	<div class="blog__item__content">
		 		<h2 class="blog__item__heading">{$post['heading']}</h2>
		 		<p class="blog__item__btn"><a href="{$blog_full_url}" title="{$post['title']}" class="btn btn--link">Read More</a></p>
		 	</div>				
		</article>
HTML;
